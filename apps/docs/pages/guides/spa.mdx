...
export default defineConfig({
  plugins: [
    ...
    csp({
      ...
      policy: definePolicy({
        scriptSrc: [self, "https://www.google-analytics.com"],    //Allow google analytics to run
        fontSrc: [self, "https://fonts.gstatic.com"],    //Allows fonts from Google to load
      }),
      build:{
        sri: true
      }
...
## HTTP headers vs. `<meta>` (v4+)

The plugin injects a `<meta http-equiv="Content-Security-Policy">` by default. Many teams prefer to send the policy as an **HTTP response header** (for `report-uri` / `report-to`, `frame-ancestors`, and to avoid duplicating large policies in HTML).

In **v4**, the optional `transformPolicy` callback receives the final built CSP string and can return `null` to skip meta injection while you copy the string into your host’s config. See the [`transformPolicy` option](/api-docs#transformpolicy) in the API docs.
